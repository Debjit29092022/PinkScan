...
def generate_pdf_report(patient_id, result, numerical_features, new_features, risk_score):
    ...
    # Set consistent margins for all pages (20mm on all sides)
    margin = 20  # 20mm margin
    page_width = 210 - (2 * margin)  # A4 width minus margins (210mm total, 190mm usable)
    page_height = 297 - (2 * margin)  # A4 height minus margins (297mm total, 257mm usable)
    
    # Enhanced PDF design with pink theme and consistent margins
    def apply_page_layout():
        pdf.set_margins(left=margin, top=margin, right=margin)
        pdf.set_line_width(0.5)
        pdf.set_draw_color(255, 105, 180)  # Pink color (#ff69b4)
        pdf.rect(margin, margin, page_width, page_height)  # Pink border within margins
    
    apply_page_layout()
    
    # Header
    pdf.set_font("Helvetica", "B", 16)
    pdf.set_text_color(255, 105, 180)  # Pink text
    ...
    # Add graphs to PDF (if files exist) on the first page, below Prediction Summary, within margins
    ...
    if os.path.exists(histogram_path):
        pdf.image(histogram_path, x=margin, y=current_y, w=page_width)
        pdf.ln(page_width + 10)  # Adjust spacing to fit within margins
    if os.path.exists(pie_chart_path):
        pdf.image(pie_chart_path, x=margin, y=current_y, w=page_width / 2 - 5)  # Half width, adjusted for margin
    if os.path.exists(bar_chart_path):
        pdf.image(bar_chart_path, x=margin + (page_width / 2) + 5, y=current_y, w=page_width / 2 - 5)  # Half width, adjusted for margin
    pdf.ln(20)  # Add extra spacing after graphs to separate from tables
    
    # Clinical Features (Improved Table Design, within margins, perfect row lines)
    pdf.set_x(margin)  # Reset x position to margin
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, "Clinical Features", 0, 1)
    pdf.set_font("Helvetica", size=10)
    col_widths = [95, 95]  # Adjusted to fit exactly within page width (190mm)
    ...
    pdf.set_fill_color(255, 255, 255)  # White for rows
    fill = False
    for i, (name, value) in enumerate(zip(new_feature_names, new_features)):
        value_str = str(value) if isinstance(value, str) else f"{float(value):.2f}" if isinstance(value, (int, float)) else str(value)
        pdf.cell(col_widths[0], 8, name, 1, 0, "L")
        pdf.cell(col_widths[1], 8, value_str, 1, 0, "L", fill)
        pdf.ln(8)
        fill = not fill  # Alternate fill color
    
    # Check if table exceeds page height, start new page if needed
    if pdf.get_y() > page_height - margin - 20:  # Leave space for footer or next section
        pdf.add_page()
        apply_page_layout()
        pdf.set_x(margin)
    
    # Numerical Features (Improved Table Design, within margins, perfect row lines)
    pdf.set_x(margin)  # Reset x position to margin
    pdf.set_font("Helvetica", "B", 12)
    ...
    pdf.set_fill_color(255, 255, 255)  # White for rows
    fill = False
    for i, (name, value) in enumerate(zip(numerical_feature_names, numerical_features)):
    ...
    # Historical Comparison and Time to Cure (if available)
    history = get_patient_history(patient_id)
    if not history.empty:
        pdf.add_page()
        apply_page_layout()  # Apply pink border and margins on new page
        
        pdf.set_x(margin)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Historical Comparison", 0, 1)
        pdf.set_font("Helvetica", size=10)
        initial_entry = history.iloc[0]  # First (initial) prediction
        current_entry = history.iloc[-1]  # Current (latest) prediction
        
        pdf.cell(0, 8, f"Initial Diagnosis: {initial_entry['prediction']}", 0, 1)
        pdf.cell(0, 8, f"Initial Risk Score: {initial_entry['risk_score']:.2f}%", 0, 1)
        pdf.cell(0, 8, f"Current Diagnosis: {current_entry['prediction']}", 0, 1)
        pdf.cell(0, 8, f"Current Risk Score: {current_entry['risk_score']:.2f}%", 0, 1)
        pdf.ln(10)

        # Feature comparison (initial vs. current, within margins, perfect row lines)
        pdf.set_x(margin)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Feature Comparison (Initial vs. Current)", 0, 1)
        pdf.set_font("Helvetica", size=10)
        col_widths = [60, 65, 65]  # Adjusted to fit exactly within page width (190mm)
        headers = ["Feature Name", "Initial Value", "Current Value"]
        ...
        fill = False
        try:
            # Try parsing as JSON first (more robust for lists)
            initial_numerical = json.loads(initial_entry['numerical_features'].replace("'", '"')) if isinstance(initial_entry['numerical_features'], str) else numerical_features
            initial_new = json.loads(initial_entry['new_features'].replace("'", '"')) if isinstance(initial_entry['new_features'], str) else new_features
        except (json.JSONDecodeError, ValueError, TypeError):
            try:
                # Fallback to ast.literal_eval for string representation of lists
                initial_numerical = ast.literal_eval(initial_entry['numerical_features']) if isinstance(initial_entry['numerical_features'], str) else numerical_features
                initial_new = ast.literal_eval(initial_entry['new_features']) if isinstance(initial_entry['new_features'], str) else new_features
            except (ValueError, SyntaxError, TypeError):
                st.warning(f"Invalid feature data for patient {patient_id}. Using current features for initial values.")
                initial_numerical = numerical_features
                initial_new = new_features  # Fallback

        # Ensure correct lengths for features
        if len(initial_numerical) != len(numerical_features) or len(initial_new) != len(new_features):
            st.warning(f"Feature length mismatch for patient {patient_id}. Using current features for initial values.")
            initial_numerical = numerical_features
            initial_new = new_features  # Fallback
        ...
        for i, (name, initial_val, current_val) in enumerate(zip(all_feature_names, initial_features, current_features)):
            if isinstance(current_val, str):
                current_val_str = current_val
            else:
                current_val_str = f"{float(current_val):.2f}" if isinstance(current_val, (int, float)) else str(current_val)
            if isinstance(initial_val, str):
                initial_val_str = initial_val
            else:
                initial_val_str = f"{float(initial_val):.2f}" if isinstance(initial_val, (int, float)) else str(initial_val)
            pdf.cell(col_widths[0], 8, name, 1, 0, "L")
            pdf.cell(col_widths[1], 8, initial_val_str, 1, 0, "C", fill)
            pdf.cell(col_widths[2], 8, current_val_str, 1, 0, "C", fill)
            pdf.ln(8)
            fill = not fill  # Alternate fill color
        ...
        # Improvement Analysis and Estimated Time to Cure
        pdf.set_x(margin)
        pdf.set_font("Helvetica", "B", 12)
        pdf.cell(0, 8, "Improvement Analysis", 0, 1)
        pdf.set_font("Helvetica", size=10)
        initial_risk = initial_entry['risk_score']
        current_risk = current_entry['risk_score']
        initial_pred = initial_entry['prediction']
        current_pred = current_entry['prediction']
        initial_date = datetime.strptime(initial_entry['date'], '%Y-%m-%d %H:%M:%S')
        current_date = datetime.strptime(current_entry['date'], '%Y-%m-%d %H:%M:%S')
        days_elapsed = (current_date - initial_date).days
        
        if initial_pred == "Malignant" and current_pred == "Benign":
            status = "Improving"
            message = "The patient's condition appears to be improving, as the tumor has shifted from malignant to benign."
            # Estimate time to cure: Assume 180–365 days for Malignant to Benign transition
            estimated_days = max(0, 365 - days_elapsed)  # Remaining days to cure (up to 1 year)
            if estimated_days > 0:
                cure_message = f"Estimated time to cure: Approximately {estimated_days} days, assuming continued treatment and monitoring."
            else:
                cure_message = "Estimated time to cure: Patient may have recovered or requires further evaluation."
        elif initial_pred == "Benign" and current_pred == "Malignant":
            status = "Worsening"
            message = "The patient's condition appears to be worsening, as the tumor has shifted from benign to malignant."
            cure_message = "Estimated time to cure: Requires immediate medical intervention; no fixed timeline available."
        elif initial_pred == current_pred:
            if current_risk < initial_risk:
                status = "Improving"
                message = f"The patient's condition is improving, with the risk score decreasing from {initial_risk:.2f}% to {current_risk:.2f}%."
                # Estimate time to cure: Assume 90–180 days for risk reduction
                risk_reduction = initial_risk - current_risk
                estimated_days = max(0, int((90 * (initial_risk - current_risk) / 100)))  # Proportional reduction (up to 90 days)
                cure_message = f"Estimated time to cure: Approximately {estimated_days} days, assuming continued improvement."
            elif current_risk > initial_risk:
                status = "Worsening"
                message = f"The patient's condition is worsening, with the risk score increasing from {initial_risk:.2f}% to {current_risk:.2f}%."
                cure_message = "Estimated time to cure: Requires immediate medical intervention; no fixed timeline available."
            else:
                status = "Stable"
                message = "The patient's condition remains stable, with no change in diagnosis or risk score."
                cure_message = "Estimated time to cure: No change detected; regular monitoring recommended."
        else:
            status = "Uncertain"
            message = "The change in diagnosis (malignant to benign or vice versa) requires further medical evaluation to determine improvement status."
            cure_message = "Estimated time to cure: Requires further medical evaluation; no fixed timeline available."

        pdf.cell(0, 8, f"Status: {status}", 0, 1)
        pdf.cell(0, 8, message, 0, 1)
        pdf.cell(0, 8, cure_message, 0, 1)
        pdf.ln(10)
    ...
    tips = [
        ...
        "Consult an oncologist for personalized treatment options based on the risk score"
    ]
    for tip in tips:
    ...
    pdf.ln(10)
    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, "Recommended Next Steps", 0, 1)
    pdf.set_font("Helvetica", size=10)
    next_steps = [
    ...
    for step in translate(next_steps):
        pdf.cell(0, 6, step.encode('latin-1', 'replace').decode('latin-1'), 0, 1)
    ...
    pdf.cell(0, 10, f"PinkScan v1.0 | Generated on {pd.Timestamp.now().strftime('%Y-%m-%d')} | Contact: support@pinkscan.ai", 0, 1, "C")
...
def plot_histogram(numerical_features, result):
    # Ensure numerical_features is a 1D array of shape (30,)
    numerical_features = np.array(numerical_features, dtype=float).flatten()
    if numerical_features.ndim != 1 or len(numerical_features) != 30:
        st.error(f"Numerical features shape mismatch: expected (30,), got {numerical_features.shape}")
        raise ValueError("Numerical features must be a 1D array with 30 values.")
    
    # Create a DataFrame with 30 rows and 30 columns (one value per feature)
    data = pd.DataFrame([numerical_features], columns=numerical_feature_names).T
    data.columns = ['Value']  # Rename column for clarity
    data['Diagnosis'] = 'Malignant' if result == "Malignant" else 'Benign'
    
    fig, axes = plt.subplots(6, 5, figsize=(20, 15))  # Grid for 30 features (6 rows, 5 columns)
    axes = axes.flatten()  # Flatten for easier indexing
    
    for i, feature in enumerate(numerical_feature_names):
        ax = axes[i]
        # Plot histogram and box plot for each feature's single value
        value = data.loc[feature, 'Value']
        data_malignant = [value] if result == "Malignant" else []
        data_benign = [value] if result == "Benign" else []
        
        # Plot histogram for each feature (single value, so use a small range)
        sns.histplot(data=data_malignant, bins=1, kde=False, color="#FF6347", label="Malignant", ax=ax, alpha=0.5)
        sns.histplot(data=data_benign, bins=1, kde=False, color="#4682B4", label="Benign", ax=ax, alpha=0.5)
        
        # Add box plot overlay for spread (single value, so minimal box)
        sns.boxplot(data=data_malignant + data_benign, ax=ax, color="#ff69b4", width=0.3, showfliers=True, flierprops={'marker': 'o', 'markersize': 5, 'markerfacecolor': 'black'})
        
        ax.legend(fontsize=8, title="Diagnosis", title_fontsize=10, loc="upper right")
...
def plot_pie(result, risk_score):
    ...
    labels = [translate("Malignant"), translate("Benign"), translate("Non-Malignant"), translate("Risk Score")]
    if result == "Malignant":
        sizes = [risk_score, 0, 0, 100 - risk_score]  # Malignant + Risk Score (100 - risk_score for Non-Malignant)
    else:
        sizes = [0, risk_score, 100 - risk_score, 0]  # Benign + Non-Malignant (100 - risk_score for Non-Malignant)
    colors = ["#FF6347", "#4682B4", "#32CD32", "#ff69b4"]
    explode = (0.1 if result == "Malignant" else 0, 0.1 if result == "Benign" else 0, 0, 0)
